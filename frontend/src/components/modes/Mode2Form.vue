    ...
    <el-card class="mode-form-card">
      ...
      <div class="card-body">
        <div class="mode-form-row">
          <div class="mode-form-group">
            <label class="mode-form-label">出口压力</label>
            <el-input v-model.number="form.hx_cold_out.p_out" type="number" step="0.1" class="mode-input-md" />
            <span class="mode-unit-text">MPa.G</span>
          </div>
          <div class="mode-form-group">
            <label class="mode-form-label">出口温度</label>
            <el-input v-model.number="form.hx_cold_out.t_out" type="number" step="1" class="mode-input-md" />
            <span class="mode-unit-text">°C</span>
          </div>
        </div>
      </div>
    ...
    <el-card class="mode-form-card">
      ...
      <div class="card-body">
        ...
        <div class="mode-form-row">
          <div class="mode-form-group">
            <label class="mode-form-label">介质类型</label>
            <el-select v-model="form.hx_hot.medium_type" placeholder="选择" @change="onMediumTypeChange('hx_hot')" class="mode-select-md">
              <el-option label="单一介质" value="single" />
              <el-option label="混合介质" value="mix" />
            </el-select>
          </div>
          ...
          <!-- 单一介质时显示介质选择 -->
          <div class="mode-form-group" v-if="form.hx_hot.medium_type === 'single'">
            <label class="mode-form-label short">介质</label>
            <el-select v-model="form.hx_hot.medium" @change="onMediumChange('hx_hot')" class="mode-select-md">
              <el-option label="氮气 (N₂)" value="N2" />
              <el-option label="氧气 (O₂)" value="O2" />
              <el-option label="空气" value="Air" />
              <el-option label="二氧化碳" value="CO2" />
          ...
          <!-- 混合介质时显示组分类型 + 5 组分 + 合计 + 归一化 -->
          <div class="mode-form-group" v-if="form.hx_hot.medium_type === 'mix'">
            <div class="mode-divider"></div>
            <div class="mode-form-group">
              <span class="mode-form-label" style="width: auto; font-size: 12px;">组分类型：</span>
              <label class="mode-radio-label">
                <input type="radio" v-model="form.hx_hot.mixData.composition_type" value="mole" />
                <span>摩尔</span>
              </label>
              <label class="mode-radio-label">
                <input type="radio" v-model="form.hx_hot.mixData.composition_type" value="mass" />
                <span>质量</span>
              </label>
            </div>
            <div class="mode-divider"></div>
            <div class="mode-mix-row">
              <div class="mode-mix-item" v-for="(value, key) in form.hx_hot.mixData.composition" :key="key">
                <input type="number" v-model.number="form.hx_hot.mixData.composition[key]" :step="key === 'H2O' || key === 'CO2' ? 0.1 : 1" />
                <span>{{ getMediumLabel(key) }}</span>
                <span class="percent">%</span>
              </div>
            </div>
            <div class="mode-mix-footer">
              <span class="mode-total-label">合计：</span>
              <span class="mode-total-value" :style="{ color: totalHxHot !== 100 ? '#f59e0b' : '#22c55e' }">{{ totalHxHot.toFixed(1) }}%</span>
              <button class="mode-normalize-btn" @click="normalizeHxHot" :disabled="totalHxHot === 0 || Math.abs(totalHxHot - 100) < 0.01">🔄</button>
            </div>
          </div>
        </div>

        <!-- 第二行：流量、入口压力、出口压力、入口温度 -->
        <div class="mode-form-row">
          <div class="mode-form-group">
            <label class="mode-form-label short">流量</label>
            <el-input v-model.number="form.hx_hot.flow_rate" type="number" class="mode-input-md" />
            <el-select v-model="form.hx_hot.flow_unit" class="mode-select-sm">
              <el-option label="T/h" value="T/h" />
              <el-option label="Nm³/h" value="Nm3/h" />
            </el-select>
          </div>
          <div class="mode-form-group">
            <label class="mode-form-label">入口压力</label>
            <el-input v-model.number="form.hx_hot.p_in" type="number" step="0.1" class="mode-input-md" />
            <span class="mode-unit-text">MPa.G</span>
          </div>
          <div class="mode-form-group">
            <label class="mode-form-label">出口压力</label>
            <el-input v-model.number="form.hx_hot.p_out" type="number" step="0.1" class="mode-input-md" />
            <span class="mode-unit-text">MPa.G</span>
          </div>
          <div class="mode-form-group">
            <label class="mode-form-label">入口温度</label>
            <el-input v-model.number="form.hx_hot.t_in" type="number" step="1" class="mode-input-md" />
            <span class="mode-unit-text">°C</span>
          </div>
        </div>
      </div>
...
const totalHxHot = computed(() => {
  return Object.values(form.hx_hot.mixData.composition).reduce((sum, val) => sum + Number(val), 0)
})
...
const normalizeHxHot = () => {
  if (totalHxHot.value === 0 || Math.abs(totalHxHot.value - 100) < 0.01) return
  const factor = 100 / totalHxHot.value
  for (const key in form.hx_hot.mixData.composition) {
    form.hx_hot.mixData.composition[key] = Math.round(form.hx_hot.mixData.composition[key] * factor * 100) / 100
  }
}
...
const submit = async () => {
  ...
  if (form.hx_hot.medium_type === 'mix') {
    if (Math.abs(totalHxHot.value - 100) > 0.01) {
  ...
  try {
    ...
    localStorage.setItem('mode2_input', JSON.stringify(inputData))
